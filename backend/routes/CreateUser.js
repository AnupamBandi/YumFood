...
const jwtSecret ="myibabvuabuvbabvbabvkabvkuba"

router.post("/createuser",
    body('email').isEmail(), body('name').isLength({ min: 5 }),
...
router.post("/loginUser", body('email').isEmail(), 
    body('password', "incorrect password").isLength({ min: 5 }), async (req, res) => {
        ...
        try {
            ...
            if (!userData) {
                return res.status(400).json({ errors: "try login with correct credenials" })
            }
